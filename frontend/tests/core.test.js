...
process.chdir(__dirname+'/..'); require('../../dist/core.js'); const N=globalThis.NS; const assert=require('assert');
let pass=0, fail=0; const T=[]; const test=(n,f)=>T.push([n,f]);
let enr=0; const user=async(email,name='Test User')=>(await N.signup({fullName:name,email,phone:'+91 9876543210',password:'password1',confirm:'password1',role:'LAWYER',registrationNumber:'D/'+(1000+(++enr))+'/2015'})).user;
...
test('signup validates and hashes passwords',async()=>{
  ...
  const dup=await N.signup({fullName:'Dup',email:'ONE@test.io',phone:'+91 9876543210',password:'password1',confirm:'password1',role:'LAWYER',registrationNumber:'D/9999/2015'}); assert(!dup.ok && dup.errors.email);
});
...
test('MCP validation and structured errors',async()=>{
  ...
  assert.strictEqual(N.mcpCall('get_claims',{case_id:'NS-1999-999'},ctx).error.code,'PERMISSION_DENIED'); assert.strictEqual(N.mcpCall('get_authority_excerpt',{authority_id:'AUTH-999'},ctx).error.code,'AUTHORITY_NOT_FOUND');
...
test('report JSON + generated PDF is structurally valid and round-trips through the PDF reader',async()=>{
  ...
  const pages=(s.match(/\/Type \/Page /g)||[]).length; assert(pages>=2); fs.writeFileSync(require('path').join(require('os').tmpdir(),'ns-sample-report.pdf'),bytes);
