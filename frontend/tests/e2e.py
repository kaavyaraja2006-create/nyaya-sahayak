...
ROOT=os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))); TESTS=ROOT+'/frontend/tests'; URL='file://'+ROOT+'/dist/index.html'; D=TESTS+'/e2e-data/'
import tempfile as _tf; S=_tf.mkdtemp(prefix='ns-shots-')+'/'
errors=[]; results=[]
...
def read_otp():
    ...
    for _ in range(200):
        ...
        if 'otp_console_delivery' in line: return json.loads(line)['code']
    raise Exception('no OTP in server log')
...
with sync_playwright() as p:
    ...
    pg.fill('#s-fullName','Kaavya Rao'); pg.fill('#s-email','kaavya@example.com'); pg.fill('#s-phone','+91 9876543210'); pg.fill('#s-password','password123'); pg.fill('#s-confirm','password123'); pg.select_option('#s-role','LAWYER'); pg.fill('#s-registrationNumber','D/1234/2015'); pg.click('#subbtn')
    if SERVER:
        pg.wait_for_selector('form[data-form=lotp-verify]',timeout=8000); ok('lawyer signup leads to the one-time code step', 'code' in pg.inner_text('.auth-form').lower())
        pg.fill('#f-otp',read_otp()); pg.click('#subbtn')
    ...
    pg.set_input_files('#docfile',files+[TESTS+'/fixture.docx',TESTS+'/fixture.pdf'])
    ...
    pg.fill('#lp','password123'); pg.click('#subbtn')
    if SERVER:
        pg.wait_for_selector('form[data-form=lotp-request]',timeout=8000); ok('password sign-in for a lawyer redirects to the one-time-code page', '/login/lawyer' in pg.url)
        pg.fill('#f-enrollmentNumber','d/1234/2015'); pg.fill('#f-mobile','9876543210'); pg.click('#subbtn'); pg.wait_for_selector('form[data-form=lotp-verify]')
        pg.fill('#f-otp','000000' if False else read_otp()); pg.click('#subbtn')
    pg.wait_for_selector('.app'); go('#/cases'); pg.wait_for_selector('#caselist tbody tr'); ok('case persists after re-login', cid in pg.inner_text('#caselist'))
